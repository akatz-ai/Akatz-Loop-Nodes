...
class SmartLoopPreviewImage(ImageNode, SaveImage):
    """
    Preview node that accumulates images across loop iterations.
    Uses display_id (stable across graph expansion) to track iterations.
    """

    _iteration_counters = {}   # display_id -> iteration count
    _accumulated_results = {}  # display_id -> list of image result dicts
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "images": ("IMAGE", ),
                "display_mode": (["latest", "gallery"], {"default": "latest"}),
            },
            "hidden": {
                "prompt": "PROMPT",
    ...
    def smart_preview_images(self, images, display_mode="latest", filename_prefix="ComfyUI",
                             prompt=None, extra_pnginfo=None, unique_id=None, dynprompt=None):
        # Get stable display_id that persists across loop iterations
        display_id = unique_id
        if dynprompt is not None:
            display_id = dynprompt.get_display_node_id(unique_id)

        # Initialize accumulators for this display_id if needed
        if display_id not in self._iteration_counters:
            self._iteration_counters[display_id] = 0
            self._accumulated_results[display_id] = []

        iteration_num = self._iteration_counters[display_id]
        self._iteration_counters[display_id] += 1
        ...
        # Track this iteration's images for "latest" mode
        current_results = []
        ...
        for (batch_number, image) in enumerate(images):
            ...
            self._accumulated_results[display_id].append(result)

        # Return based on display mode
        ui_images = current_results if display_mode == "latest" else self._accumulated_results[display_id]
        return {
            "ui": {"images": ui_images},
            "result": (images,),
        }
    ...
    @classmethod
    def reset_accumulator(cls, display_id=None):
        """Reset accumulators. Call on new prompt execution."""
        if display_id:
            cls._iteration_counters.pop(display_id, None)
            cls._accumulated_results.pop(display_id, None)
        else:
            cls._iteration_counters.clear()
            cls._accumulated_results.clear()
